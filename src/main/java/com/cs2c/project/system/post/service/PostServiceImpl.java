package com.cs2c.project.system.post.service;
...
public class PostServiceImpl implements IPostService
{
    ...
    public AjaxResult downloadF() {
        List<Post> list = selectPostList(new Post());
        String stdout = "";
        for(Post post : list) {
            stdout += post.getPostId()+"\t";
            if(post.getPostCode().length() >= 8) stdout +=post.getPostCode()+"\t";
            else stdout +=post.getPostCode()+"\t\t";
            
            if(post.getPostName().length() >= 5) stdout +=post.getPostName()+"\t";
            else stdout +=post.getPostName()+"\t\t";

            stdout += post.getCreateTime()+"\n";
        }
        //System.out.println("stdout :"+stdout);
       
        Calendar cal = Calendar.getInstance(); 
        //System.out.println("Calendar");
        String fname = "岗位管理-"+ (cal.get(Calendar.MONTH)+1)+"-"+cal.get(Calendar.DAY_OF_MONTH)+".zip";
