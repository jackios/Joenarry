package com.cs2c.project.system.role.service;
...
import com.cs2c.project.system.role.domain.RoleDept;
import com.cs2c.project.system.role.domain.RoleMenu;
import com.cs2c.project.system.role.mapper.RoleDeptMapper;
import com.cs2c.project.system.role.mapper.RoleMapper;
import com.cs2c.project.system.role.mapper.RoleMenuMapper;
...
public class RoleServiceImpl implements IRoleService
{
    ...
    public AjaxResult downloadF() {
        List<Role> list = selectRoleList(new Role());
        String stdout = "";
        for(Role role : list) {
            stdout += role.getRoleId()+"\t";
            if(role.getRoleName().length() >= 5) stdout +=role.getRoleName()+"\t";
            else stdout +=role.getRoleName()+"\t\t";
            
            if(role.getRoleKey().length() >= 8) stdout +=role.getRoleKey()+"\t";
            else stdout +=role.getRoleKey()+"\t\t";

            stdout += role.getCreateTime()+"\n";
        }
        //System.out.println("stdout :"+stdout);
       
        Calendar cal = Calendar.getInstance(); 
        //System.out.println("Calendar");
        String fname = "角色管理-"+ (cal.get(Calendar.MONTH)+1)+"-"+cal.get(Calendar.DAY_OF_MONTH)+".zip";
