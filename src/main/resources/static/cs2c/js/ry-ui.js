...
(function ($) {
    $.extend({
    	...
    	table: {
            ...
            _customParams: {},
			// 自定义初始化表格
            customInit: function(options, itemId, toolId) {
            	var tableId = "#" + itemId;
            	var toolbarId = "#" + toolId;
                $.table._customOption = options;
                $.table._customParams = $.common.isEmpty(options.queryParams) ? $.table.queryParams : options.queryParams;
        ...
        // 操作封装处理
        operate: {
    		freshCurrentTab: function() {
                var top = $($(parent.parent.parent.document).contents());
                var currentId = top.find('.page-tabs-content').find('.active').attr('data-id');
                var target = top.find('.Cs2c_iframe[data-id="' + currentId + '"]');
                var url = target.attr('src');
                target.attr('src', url).ready();
			},

            // 提交数据
            submitAndRefreshCurrentTab: function(url, type, dataType, data) {
            ...
            // post请求传输
            postAndRefresh: function(url, data) {
                $.operate.submitAndRefreshCurrentTab(url, "post", "json", data);
            },
            ...
            // 版本管理
            reversion: function(id) {
                var url = $.common.isEmpty(id) ? $.table._option.createUrl : $.table._option.createUrl.replace("{id}", id);
                $.modal.open("核心配置-版本管理", url);
            },
            // 上传文件
            general: function(title, flag) {
                var url = $.common.isEmpty(flag) ? $.table._option.generalUrl.replace("/{flag}", "") : $.table._option.generalUrl.replace("{flag}", flag);
                $.modal.open(title, url);
            },
            // 查看特定版本的配置信息
            viewVersionInfo: function(id) {
                var url = $.common.isEmpty(id) ? $.table._option.viewVersionUrl.replace("/{id}", "") : $.table._option.viewVersionUrl.replace("{id}", id);
                $.modal.open("核心配置-版本配置详情", url);
            },
            // 修改信息
            edit: function(id) {
            ...
            // 修改信息
            editCustomTitle: function(id, title) {
                var url = $.table._option.updateUrl.replace("{id}", id);
                $.modal.open("" + title, url);
            },
            // 添加信息 全屏
            ...
            // 生成预览
            genPreview: function(url, dir, filename) {
                $.modal.loading("正在处理中，请稍后...");
                var config = {
                    url: url,
                    type: "post",
